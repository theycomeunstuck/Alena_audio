...
def parse_document(file_path: Path) -> dict:
    ...
    if not topic:
        # No `TOPIC:` header (e.g. a plain .rtf/.docx) — fall back to the file name
        # so the row still has a meaningful, non-empty topic label instead of "".
...
def ingest_pdf(pdf_path: Path, subject: str, topic_prefix: str,
    ...
    # Clamp to the document bounds so an over-specified range (e.g. 1-100 on a
    # 50-page PDF) does not raise IndexError on doc[page_num].
    ...
    try:
        for page_num in range(start_page, end_page):
            ...
            try:
                ...
                # Commit per page: a failure on page N must not roll back the pages
                # already processed. upsert_pdf_page is idempotent, so a re-run recovers.
                conn.commit()
                stats["new" if result == "insert" else "updated"] += 1
            except Exception as e:
                conn.rollback()
                stats["failed"] += 1
                print(f"  p.{page_idx}: FAILED ({e.__class__.__name__}: {e}) — пропускаем")
                continue
    finally:
        conn.close()

    doc.close()
    total = stats["new"] + stats["updated"] + stats["skipped"] + stats["failed"]
    print(f"Done: {pdf_path.name} — {total} pages "
          f"({stats['new']} new, {stats['updated']} updated, "
          f"{stats['skipped']} blank, {stats['failed']} failed)")
