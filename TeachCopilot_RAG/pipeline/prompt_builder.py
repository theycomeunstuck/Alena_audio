...
from pipeline.config import SPEAKER_MODE
...
def _append_rag(parts: list[str], rag: list[dict]) -> None:
    """Append a grounded <relevant_material> block. Used by both the child and
    the adult prompt so retrieved material is always presented as optional
    context ('use only if relevant; do not invent facts')."""
    if not rag:
        return
    parts.append("\n<relevant_material>")
    parts.append("Правила_использования: используй эти материалы только если они релевантны вопросу; не выдумывай факты, которых здесь нет.")
    for r in rag:
...
def build_system_prompt(base: str, profile: dict | None, rag: list[dict],
    ...
    if speaker_type == "adult" and SPEAKER_MODE == "mapped_or_adult":
    ...
    # Интересы рендерятся ниже в <child_learning_profile> корректным join'ом.
    # Раньше здесь был второй, битый вывод сырого списка (['a','b']) — удалён.
...
def _build_adult_prompt(profile: dict | None, rag: list[dict]) -> str:
    ...
    # Ground lesson-content answers: adult_prompt.txt promises answering about
    # lesson content, which must come from retrieved material, not parametric memory.
    _append_rag(parts, rag)
